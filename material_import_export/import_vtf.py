...
def import_texture_from_vpk(file, name, load_alpha=True, alpha_only=False):
    print('Loading {}'.format(name))
    vtf_lib.image_load_from_mem(file)
    ...
    if (vtf_lib.get_image_flags().get_flag(ImageFlag.ImageFlagEightBitAlpha) or vtf_lib.get_image_flags().get_flag(
        ...
        has_alpha = int(alpha_view.sum(dtype=np.double)
                        ) != int(alpha_view.shape[0] * 255)
